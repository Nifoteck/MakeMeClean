...
export default function AdminPanel() {
  ...
  const fetchBookings = async () => {
    ...
    const [{ data }, { data: servicesData }] = await Promise.all([
      supabase.from("bookings").select("*, profiles(full_name, phone)").order("date", { ascending: false }),
          ...
          {filtered.map((b) => (
            <div key={b.id} className="grid md:grid-cols-12 gap-2 px-5 py-4 border-t border-gray-100 items-center hover:bg-gray-50 transition-colors">
              ...
              <div className="md:col-span-3 flex items-center gap-2">
                <div className="w-9 h-9 rounded-lg overflow-hidden bg-green-50 border border-green-100 shrink-0">
