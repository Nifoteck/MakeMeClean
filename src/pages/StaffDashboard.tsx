...
import {
  Calendar, Clock, MapPin, LogOut, CalendarDays, CheckCircle2,
  Banknote, Layers, XCircle,
} from "lucide-react";
...
export default function StaffDashboard() {
  ...
  const fetchAssigned = async (staffId: string) => {
    ...
    const [{ data }, { data: servicesData }] = await Promise.all([
      supabase
        ...
        .select("id, acceptance_status, decline_reason, bookings(id, service_name, service_type, date, time_slot, address, city, postcode, price, status, payment_status)")
        .eq("staff_id", staffId)
  ...
  const acceptShift = async (assignmentId: string, bookingId: string) => {
    ...
    await supabase
      ...
      .eq("id", assignmentId);
    setBookings((prev) =>
      prev.map((b) => b.id === bookingId ? { ...b, acceptanceStatus: "accepted" } : b)
    );
    setActionId(null);
  };
  ...
  const declineShift = async (assignmentId: string, bookingId: string) => {
    ...
    await supabase
      .from("booking_assignments")
      .update({ acceptance_status: "declined", decline_reason: declineText || null })
      .eq("id", assignmentId);
    setBookings((prev) =>
      prev.map((b) =>
        b.id === bookingId
          ? { ...b, acceptanceStatus: "declined", declineReason: declineText || null }
          : b
      )
    );
    setDecliningId(null);
  ...
  const statusColor = (s: string) =>
    s === "upcoming"  ? "bg-green-50 text-green-700 border-green-100"  :
    s === "completed" ? "bg-gray-100 text-gray-600 border-gray-200"    :
    s === "cancelled" ? "bg-red-50 text-red-600 border-red-100"        :
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-10 max-w-md w-full text-center">
          ...
          <h1 className="text-xl font-black text-gray-900 mb-2">No staff profile found</h1>
          <p className="text-sm text-gray-500 mb-1">Your account isn't linked to a staff profile yet.</p>
          <p className="text-xs text-gray-400 mb-6">Ask an admin to assign your account.</p>
          <button
            onClick={signOut}
  ...
  return (
    <StaffLayout title="My Shifts" subtitle={`Welcome back, ${staff.first_name}`}>

      {/* Stats */}
      <div className="grid sm:grid-cols-3 gap-4 mb-8">
        {[
          { label: "Total assigned",  value: bookings.length,  icon: Layers,       color: "bg-slate-100 text-slate-600"     },
          { label: "Upcoming shifts", value: upcoming.length,  icon: CalendarDays, color: "bg-green-100 text-green-700"     },
          { label: "Completed",       value: completed.length, icon: CheckCircle2, color: "bg-emerald-100 text-emerald-700" },
      ...
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs font-semibold text-gray-400">{upcoming.length} total</span>
      </div>
      ...
      {upcoming.length === 0 ? (
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm py-20 text-center">
          ...
          <p className="text-xs text-gray-400 mt-1">You'll see new jobs here when assigned.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {paginated.map((b) => (
            <div key={b.id} className="bg-white border border-gray-100 rounded-2xl shadow-sm p-5 hover:border-gray-200 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3 flex-wrap">
                    <div>
                      ...
                      <span className={cn("inline-block text-xs font-semibold px-2.5 py-0.5 rounded-full border mt-1", statusColor(b.status))}>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    {b.acceptanceStatus === "accepted" ? (
                      <div className="flex items-center gap-2 text-xs font-semibold text-green-600">
                        <CheckCircle2 className="w-4 h-4" /> You accepted this shift
                      </div>
                    ) : b.acceptanceStatus === "declined" ? (
                      <div>
                        <div className="flex items-center gap-2 text-xs font-semibold text-red-500">
                          <XCircle className="w-4 h-4" /> You declined this shift
                        </div>
                        {b.declineReason && (
                          <p className="text-xs text-gray-400 mt-0.5 ml-6">Reason: {b.declineReason}</p>
                        )}
                      </div>
                    ) : decliningId === b.id ? (
                      <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                          ...
                          >
                            {actionId === b.id ? "Declining…" : "Confirm Decline"}
                          </button>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-amber-50 border border-amber-100 rounded-xl p-3">
                        <p className="text-xs font-semibold text-amber-700 mb-2">Confirm this shift?</p>
                        <div className="flex gap-2">
                          <button
                            onClick={() => acceptShift(b.assignmentId, b.id)}
                            disabled={actionId === b.id}
                          ...
                          <button
                            onClick={() => { setDecliningId(b.id); setDeclineText(""); }}
                            disabled={actionId === b.id}
          ...
          <Pagination page={page} total={upcoming.length} pageSize={PAGE_SIZE} onChange={setPage} />
        </div>
      )}
    </StaffLayout>
  );
