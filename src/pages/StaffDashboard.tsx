  ...
  const fetchAssigned = async (staffId: string) => {
    ...
    const [{ data }, { data: servicesData }] = await Promise.all([
      supabase
        ...
        .select("status, bookings(id, service_name, service_type, date, time_slot, address, city, postcode, price, status, payment_status)")
        .eq("staff_id", staffId)
              ...
              <div key={b.id} className="card">
                <div className="flex items-start justify-between gap-4 flex-wrap">
                  <div className="flex items-start gap-3">
                    <div className="w-10 h-10 rounded-xl overflow-hidden bg-green-50 border border-green-100 shrink-0 mt-0.5">
