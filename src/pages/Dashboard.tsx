...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    ]).then(([{ data: b }, { data: p }, { data: s }, { data: r }]) => {
      setBookings(b ?? []);
      setProfile(p);
      const imgMap: Record<string, string> = {};
      for (const svc of s ?? []) if (svc.image_url) imgMap[svc.id] = svc.image_url;
      setServiceImages(imgMap);
      setReviewedIds(new Set((r ?? []).map((x: any) => x.booking_id)));
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="flex items-center gap-4 mb-8">
          <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white text-lg font-black shrink-0 shadow-sm"
            style={{ background: "linear-gradient(135deg,#16a34a,#15803d)" }}>
            {initial}
          </div>
          <div>
            <h1 className="text-2xl font-black text-gray-900 tracking-tight">Hello, {displayName}</h1>
        ...
        <div className="bg-white border border-gray-100 rounded-2xl shadow-sm overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-50">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-black text-gray-900 uppercase tracking-wide">Recent Bookings</h2>
            </div>
            <Link href="/bookings" className="text-xs font-semibold text-green-600 hover:underline flex items-center gap-1">
          ...
          {bookings.length === 0 ? (
            <div className="text-center py-16">
              ...
              <Link href="/book" className="btn-primary inline-block mt-4 text-sm">
